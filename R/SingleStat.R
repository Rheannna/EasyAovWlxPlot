...
SingleStat = function(data = data_wt,plot = "bar",method_Mc = "Tukey",i= 4,sig_show ="abc"){
  ...
  if (plot == "bar") {
    PlotresultBar = aovMuiBarPlot(data = data_wt, i= 3,sig_show =sig_show,result = result[[1]])
    p = PlotresultBar[[1]]

  } else if (plot == "box"){
    PlotresultBox = aovMuiBoxP(data = data_wt, i= 3,sig_show =sig_show,result = result[[1]])
    p = PlotresultBox[[1]]
